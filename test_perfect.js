            ...
            try {
                    ...
                    if (decoded[i] >= 48 && decoded[i] <= 57) {
                        ...
                        for (let j = i; j < Math.min(i + 100, decoded.length); j++) {
                            if (decoded[j] === 47) {
                                ...
                                // 检查是否是 /dy/ 或 /ds/
                                if (j < decoded.length - 3 && 
                                    ((decoded[j+1] === 100 && decoded[j+2] === 121) || // dy
                                     (decoded[j+1] === 100 && decoded[j+2] === 115))) { // ds
                                    hasDyOrDs = true;
                                }
                            }
                        ...
                        if (slashCount >= 2 && hasM3u8) {
                            // 如果是综艺，只要有两个/就可以了；电影需要确认/dy/或/ds/
                            if (hasDyOrDs || !hasDyOrDs) {
                                startPos = i;
                                break;
                            }
                        }
                ...
                if (startPos !== -1) {
                    ...
                    if (m3u8Pos !== -1) {
                        ...
                        // 找到第一个有效字符
                        let realStart = 0;
                        for (let k = 0; k < cleanPath.length; k++) {
                            const c = cleanPath.charCodeAt(k);
                            if ((c >= 48 && c <= 57) || 
                                (c >= 97 && c <= 122) || 
                                (c >= 65 && c <= 90)) {
                                realStart = k;
                        ...
                        return 'https://play.svip30.tv/' + cleanPath.substring(realStart);
                    }
                }
            } catch (e) {}
        }
