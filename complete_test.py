...
class CompleteSpider(Spider):
    ...
    def init(self, extend=""):
        self.url = 'https://www.4kmovie.top'  # 去掉反引号
    ...
    def homeVideoContent(self):
        try:
            res = requests.get(self.url, headers=self.headers, timeout=10)  # 延长超时时间
            ...
            print(f"homeVideoContent Error: {e}")  # 打印异常信息
            return {'list': []}
...
if __name__ == "__main__":
    print("=== 测试完整爬虫 ===")
    spider = CompleteSpider()
    spider.init()
    
    print("\n1. 测试 homeContent:")
    home_content = spider.homeContent(False)
    print(f"   分类数量: {len(home_content['class'])}")
    print(f"   分类列表: {[c['type_name'] for c in home_content['class']]}")
    
    print("\n2. 测试 homeVideoContent:")
    home_videos = spider.homeVideoContent()
    print(f"   找到 {len(home_videos['list'])} 个视频")
    if home_videos['list']:
        print(f"   第一个视频: {json.dumps(home_videos['list'][0], ensure_ascii=False, indent=4)}")
    
    print("\n3. 测试 searchContent:")
    search_videos = spider.searchContent('良陈美锦', False, 1)
    print(f"   找到 {len(search_videos['list'])} 个结果")
    if search_videos['list']:
        print(f"   第一个结果: {json.dumps(search_videos['list'][0], ensure_ascii=False, indent=4)}")
