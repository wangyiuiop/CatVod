...
class MySpider(Spider):
    ...
    def detailContent(self, array):
        vod_id = array[0]
        res = requests.get(f"{self.url}/voddetail/{vod_id}.html", headers=self.headers, timeout=5)
        soup = BeautifulSoup(res.text, 'html.parser')
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        for item in soup.select('.module-card-item'):
            ...
            pic_img = item.select_one('.module-item-pic img')
            
            if not title_a or not pic_img:
                continue
            ...
            pic = pic_img.get('data-original') or pic_img.get('src') or ''
    ...
    def playerContent(self, flag, id, vipFlags):
        try:
            ...
            res = requests.get(play_link, headers=self.headers, timeout=5)
            html = res.text
            ...
            try:
                ...
                svip_html = requests.get(svip_url, headers=svip_headers, timeout=5).text
                ...
                if token_match:
                    ...
                    api_res_str = requests.get(api_url, headers=api_headers, timeout=5).text
...
if __name__ == "__main__":
    ...
    print("Testing searchContent with keyword '良陈美锦'...")
